...
<script>
    ...
    function doPage(pages) {
        var $page = $("#page").page({
            ...
            jump: function (context, first) {
                ...
                $("#content").html(dealData(context.option.curr, data, nums));
            }
        });
    }
    ...
    function dealData(curr, alldata) {
        ...
        for (var i = start; i <= last; i++) {
            str += '<tr>\n' +
                '                        <td>' + getStringWithspace(data[i].name) + '</td>\n' +
                '                        <td>' + getStringWithspace(data[i].phone) + '</td>\n' +
                '                        <td>' + getSex(data[i].sex) + '</td>\n' +
                '                        <td>' + getStringWithspace(data[i].age) + '</td>\n' +
                '                        <td>' + getStringWithspace(data[i].wantMoney) + '</td>\n' +
                '                        <td>' + getDate(data[i].wantTime) + '</td>\n' +
                '                        <td>' + getStringWithspace(data[i].useType) + '</td>\n' +
                '                        <td>' + getUseCycle(data[i].useCycleYear, data[i].useCycleMonth, data[i].useCycleDay) + '</td>\n' +
                '                        <td>' + getRepaymentType(data[i].repaymentType) + '</td>\n' +
                '                        <td>' + getStringWithspace(data[i].rank) + '</td>\n' +
                '                        <td>' + getStringWithspace(data[i].idcard) + '</td>\n' +
                //                '                        <td>'+getStringWithspace(data[i].state)+'</td>\n' +
                '                        <td>' + getStringWithspace(data[i].process) + '</td>\n' +
                '                        <td>\n' +
                //                '                            <button class="am-btn am-btn-default am-btn-xs am-text-secondary"><span\n' +
                //                '                                    class="am-icon-pencil-square-o"></span> 详情\n' +
                //                '                            </button>\n' +
                '                            <button class="am-btn am-btn-default am-btn-xs"><span\n' +
                '                                    class="am-icon-copy"></span> 编辑进度\n' +
                '                            </button>\n' +
                '                        </td>\n' +
                '                    </tr>';
        }
