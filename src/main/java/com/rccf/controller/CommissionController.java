...
//                "from `employee`  e  WHERE  (e.`role` =3  or e.`role` =2)  and e.`state` =1   and  'yeji' > 0 ORDER BY yeji DESC ";
        String sql = "SELECT * FROM\n" +
                ...
                "      (SELECT  sum(a.`service_fee_actual`)  FROM `accepted` a WHERE a.`end_date`  >= '2017-10-01' and a.`end_date`< '2017-11-01'   and  a.`clerk`=e.code  AND a.`state` = 2)  yeji\n" +
                "    from `employee`  e\n" +
...
//        return array.toJSONString();
    ...
    @ResponseBody
    @RequestMapping(value = "/personalCommission")
    public String personalCommission(HttpServletRequest request) {
        String sql = "SELECT e.`id` ,e.`name` ,e.`code` , e.`department`,  1 as length,\n" +
        ...
        return ResponseUtil.fail(0, "没有查询到有个人业绩");
    }
    ...
    public String getEmployeeCommission(HttpServletRequest request, HttpServletResponse reponse) {
        String sql = "SELECT e.id ,e.name,e.department,e.role,e.dupty_director_name,\n" +
