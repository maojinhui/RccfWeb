        ...
        try {
            ...
            if (acceptedService.saveOrUpdate(accepted)) {
                ...
                try {
                    ...
                    AcceptChannelInfo channelInfo = null;
                    for (int i = 0; i < array.size(); i++) {
                        channelInfo = new AcceptChannelInfo();
                        channelInfo.setAcceptId(accept_id);
                        object = array.getJSONObject(i);
                        String agengcy_name = object.getString("agengcy_name");
                        String product_name = object.getString("product_name");
                        String contact_name = object.getString("contact_name");
                        String loan_amount = object.getString("loan_amount");
                        String face_sign = object.getString("face_sign");
                        String content = object.getString("content");
                        channelInfo.setAgencyName(agengcy_name);
                        channelInfo.setProductName(product_name);
                        channelInfo.setContactName(contact_name);
                        if(Strings.isNullOrEmpty(loan_amount)){
                            channelInfo.setLoanAmount(0d);
                        }else {
                            channelInfo.setLoanAmount(Double.valueOf(loan_amount));
                        }
                        channelInfo.setFaceSign(face_sign);
                        channelInfo.setContent(content);
                        boolean save = baseService.save(channelInfo);
                        if (!save) {
            ...
            e.printStackTrace();
            return ResponseUtil.fail(0,e.toString());
        }
