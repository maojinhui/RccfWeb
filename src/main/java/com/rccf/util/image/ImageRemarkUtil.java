...
//    private static String fontPath = ImageRemarkUtil.class.getClassLoader().getResource("/").getFile();
//    private static String fontPath =Thread.currentThread().getContextClassLoader().getResource("/fonts/msyh.ttf").toString();
...
//    private static Font font1 = Font.createFont(Font.TRUETYPE_FONT,new File(fontPath));
    ...
    public static String markImageByText(String logoText, String srcImgPath,
        ...
        try {
            ...
            // 6、设置水印文字Font
            g.setFont(font);
//            g.setFont(Font.createFont(Font.TRUETYPE_FONT,new FileInputStream(new File(fontPath))));
            // 7、设置水印文字透明度
            g.setComposite(AlphaComposite.getInstance(AlphaComposite.SRC_ATOP,
                    alpha));
            // 8、第一参数->设置的内容，后面两个参数->文字在图片上的坐标位置(x,y)
  ...
  /**
     ...
     * @param degree
     */
    public static String markImageByText(String logoText, String srcImgPath,Font font,
                                         String targerPath, Integer degree,
                                         int positionWidth, int positionHeight) {
